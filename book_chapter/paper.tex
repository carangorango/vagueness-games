...
The picture sketched in the last paragraph neglects one crucial thing: learning from observation. 
For rationality, we need to consider a rational agent's ability to learn from previous experience and to generalize from prior experience. 
For evolutionary adaptation, the story so far misses the key fact that languages and practices have to be learnable in appropriate time from the actual observables, lest what is learned and further transmitted will simply be a different language or practice (\mf{insert refs for iterated learning}).
But even if every language learner in history had been perfectly rational, it is unlikely that anyone ever succeeded in reducing all uncertainty about language use in context and say with confidence: this is the threshold that truly separates heaps from non-heaps (\mf{ref / footnote Williamson?}). 


 Which strategies for generalization are best depends on the stochastic properties of the environment in which learning and decision making takes place. The problem for language is that it what works best for on agent depends on what the other agent does. Communication problems are games in the formal sense of delivering payoffs only as a function of what all players do. Consequently, the problem of how best to adapt behavior from past experience in language games also depends on how other agents categorize, generalize and behave. Nothing gets off the ground with hyper-rationality alone. There needs to be learning, evolution and circumstances. As with sim-max games above, biases and other quirks of general cognition are likely only helpful in solving this massive coordination problem. Maybe our world is a place where particular learning biases are needed for any regular and shared communicative practice to get off the ground.
