...
The motivation is as follows: imagine agents who are born with varying perceptual abilities, subsequently learn a signaling strategy by imitation of other agents (either within or across populations), and depending on the success of the strategies they develop are more or less likely to survive and reproduce.
...
Formally, let's define $A$ to be the set of imprecision values considered.
...
Regarding the duration of the simulations, due to the mixing in of random strategies at each step in order to simulate new individuals being introduced into the population (birth rate was fixed at $\gamma = 0.05$), the convergence criteria is no longer applicable.
