...
We start, in Section~\ref{sec:facets-rationality}, by clarifying some concepts that can help characterize different notions of rationality.
In Section~\ref{sec:signaling-games} we introduce the framework of signaling games and in Section~\ref{sec:sim-max-vagueness} we look into explanations of vagueness in a special kind of signaling game, a so-called sim-max game.
Section~\ref{sec:finite-experience} reflects on the implications of these explanations for notions of rationality.
...
\section{Facets of rationality}
\label{sec:facets-rationality}

Rationality is an elusive notion which is frequently debated in areas like philosophy, psychology, and economics.
Discussions around it, although touching on different aspects of the concept, often rely on a number of implicit assumptions.
We can start by considering what is deemed to be most important when characterizing the rationality of a given choice.
The aforementioned classical picture of language and meaning is focused on procedures.
Thus, a choice is deemed rational if it was arrived at by means of following proper rules of reasoning, like for example a logically valid deductive inference.
A \emph{procedural} account of rationality focuses on the means, rather than the ends.
One can also do the opposite and focus on the consequences of the choice instead.
\emph{Instrumental} rationality, as it is typically called\todo{references}, characterizes an agent's choice as rational if it maximizes the possibility of achieving a desired goal, regardless of the means.
The notion is linked to David Hume~\parencite*{hume_treatise_1738} and epitomized in his assertion that ``Reason is, and ought to only be the slave of the passions.''
...
\footnote{\todo[inline]{discuss other options like minimax and such}}%
.
...
When developing models where rationality is relevant, be it constructing a logical system or setting up a formal game, the assumed epistemic relation between whatever agents are involved and the elements of the model can come to bear on considerations of rationality.
Depending on how accurate and complete the agent's knowledge of its environment, the goals to be achieved, the choices or rules available, the relation between those choices and the objectives, etc, the verdict over the rationality of certain choices can vary.
...
This is important because, hypothetically, there could be choice mechanisms that are sub-optimal at a local level (for each situation) but are actually perfectly rational at a global level.
...
Dynamic, epistemic, fuzzy, paraconsistent, and other types of logic\todo{cite Modal Logic for Open Minds?}, can potentially capture procedural rationality with limited non-strategic agents.
Game-theoretical models of language can, on the other hand, also assume local instrumental rationality with omnisciente highly strategic agents.
...
The vocabulary introduced in this section will, we hope, help inform the discussion that follows.
...
There is a focus, not on meaning as a kind of correspondence, but rather on the use of signals for specific purposes \parencite[but see][]{Hutteger:2007_Evol_Indicatives_Imperatives,Harms2010:Determining-tru,skyrms_signals_2010,Franke2013:An-adaptationis}.
...
The models of~\textcite{franke_vagueness_2011}, generalized reinforcement learning~\parencite{oconnor_evolution_2014}, and the work of~\textcite{franke_vagueness_2017}, all paint a picture of agents with a basic level of instrumental rationality, possibly limited awareness of the game and a lack of strategic capabilities, adapting their behavior with only short-term gains in sight.
...
Despite the plausibility of the mechanisms proposed (limited memory, imprecise calculation of expected utilities, generalization, state confusability), this feels like a bittersweet conclusion because of this hypothetical possibility of an ideal strategy that seems to be only artificially barred from the agents.
