...
the actual state, so to speak. The utility function should be a monotonically increasing
function of similarity.
A sender's behavior can be represented by a pure strategy $\Spure \in \Messgs^\States$, which deterministically defines which message would be used for each state, a mixed strategy $\Smixed \in \Delta(\Messgs^\States)$, which characterizes a probability distribution over pure strategies, or a behavioral strategy $\Sstrat \in \Delta(\Messgs)^\States$, which defines the probability that a given message would be used for each state.
The analogous happens for the receiver's behavior.
A play in such a game with behavioral strategies is illustrated in Figure~\ref{fig:sim-max-illustration}.
...
\begin{figure}
  ...
  \caption{A round of play in a sim-max game with behavioral strategies: Nature (N) chooses a state $\mystate{1}$ with probability $\Pr(\mystate{1})$; a sender (S) selects a message $\messg$ with probability $\Sstrat(\messg \probbar \mystate{1})$; a receiver (R) selects a state $\mystate{2}$ with probability $\Rstrat(\mystate{2} \probbar \messg)$. Payoff for both sender and receiver is given by $\Utils(\mystate{1}, \mystate{2})$.}
...
$\Util \mycolon \States \times \States \rightarrow \mathds{R}$ the utility
...
Appendix~\ref{sec:formal-stuff} for a derivation of the standard replicator dynamic from this update scheme).
Every now and then a random agent gets a chance to alter his behavior for one of his choice
points. Say, this is a sender who gets to ``reconsider'' his choice of message for state
$\state$. The revising agent then observes what some random agent does at $\state$, say
$\messg$, and will henceforth play $\messg$ in $\state$ with a probability given by the
...
assumption, especially when some states can be fairly similar and therefore likely to be
...
focus on the main ideas. Formal detail is provided by Appendix~\ref{sec:formal-stuff}. Take a sender who gets to
revise behavior for state $\state$. What agents can plausibly revise by imitation is their pure
strategy, which maps perceived states onto messages. We should therefore assume that when an
agent gets to revise his strategy for the perceived state $\state$, this need not necessarily
be the actual state. Moreover, when that agent observes $\state$, another possibly
to-be-imitated agent may yet perceive a completely different state altogether. Given what that
latter agent perceives, his actual (pure) strategy will determine what he plays. In sum, to
describe how likely the potential imitator observes a message choice $\messg$, we are
interested in the conditional probability $P_o(\messg \probbar \state)$ that some other random
agent realizes $\messg$ when the first agent perceived $\state$. This
$P_o(\messg \probbar \state)$ is derived from the prior probability of states, the current
sender population behavior $\Sstrat$ and the given observation noise (see
Appendix~\ref{sec:formal-stuff}). Eventually, the imitating agent adopts $\messg$ as his choice for $\state$ with
probability given by the expected utility of sending $\messg$ when perceiving state
$\state$. Expected utility should, of course, take the probabilistic confusability of states
into account as well.

Similar considerations apply to the receiver side. If an agent who gets a chance to change his
...
Appendix~\ref{sec:formal-stuff} shows how imprecise imitation of this sort leads to mean changes in population
...
interval, including $0$ and $1$. All states occur, for simplicity, with the same probability (\emph{i.e.}~$P_a$ is uniform).
...
\begin{align*}
 ...
 \expo \left ( -  \frac{\card{\mystate{i} - \mystate{j}}^2}{ \toler^2} \right ) & \textrm{otherwise.} \\
    \end{cases}
\end{align*}
...
arbitrary) degree of stability. In other words, out notion of ``convergence'' is a measure of
relative speed: it is true that the system reached a state in which evolutionary adaptations
...
the presence of noise, that show the hallmarks of vagueness as desired. It also gives rise to
...
explain the existence of vagueness in a language even if we assume each agent commands a
non-vague idiolect.
...
would be equivalent to our present proposal in all respects.
...
side-effect. A certain degree of imprecision can lead to higher \emph{within group expected utility},
...
derive the standard replicator dynamic from noise-free imitation of success. The, in
...
\end{align*}
This latter formulation is the continuous-time formulation of the replicator
dynamic. We obtain the discrete-time formulation from it by assuming that discrete update steps
are infinitesimally small, so that:
 \begin{align*}
...
Section~\ref{sec:repl-diff-dynam} in Equation~(\ref{eq:nRD}). To see how the discrete-time
formulation gives rise to the continuous-time formulations above, let's assume that update
steps are infinitesimally small, so that, for the sender case:
\begin{align*}
  \dot{\sigma}(\messg \probbar \state) & = \sigma'(\messg \probbar \state) - \sigma(\messg
  \probbar \state)\\
