...
\begin{figure}
  ...
  \caption{A round of play in a sim-max game: Nature (N) chooses a state $\mystate{1}$ with
    probability $\Pr(\mystate{1})$; a sender (S) selects a message
    $\messg = \Spure(\mystate{1})$ determined by his pure strategy $\Spure$; a receiver (R)
    selects a state $\mystate{2} = \Rpure(\messg)$ determined by her pure strategy
    $\Rpure$. Payoff for both sender and receiver is given by
