...
sender of choosing message $\messg$, given state $\state$ and a receiver
...
utility of the latter agents' choice. Since the sim-max games we will be looking at in
Section~\ref{sec:exploring-rdd} have positive utilities, we can identify the switching
probability with the expected utility.

Let's consider sender strategies, as the receiver case is parallel. Call the
agent who gets a chance to change behavior ``learner'' and the possibly to-be-imitated agent
``model.'' A random learner is drawn from the population and given a chance to change
behavior at choice point $\state$. The probability that our learner plays $\messg$ is
$\Sstrat(\messg \probbar \state)$. The learner observes what a randomly sampled model does at
...
\begin{align*}
  ...
  \underbrace{\Sstrat(\messg \probbar \state)}_{\text{model plays $\messg$}} \cdot
  \underbrace{\EU(\messg, \state, \Rstrat)}_{\text{EU model choice}}
  % P(\messg \rightarrow m', \state) & = \sum_{\messg'} \underbrace{\Sstrat(\messg \probbar
  %   \state)}_{\text{learner plays $\messg$}} \cdot
  % \underbrace{\Sstrat(\messg' \probbar \state)}_{\text{model plays $\messg'$}} \cdot
  % \underbrace{\EU(\messg', \state, \Rstrat)}_{\text{EU model choice}}
...
the dynamic that ensues from imitation of success. Since imprecision works slightly
...
observed $\mystate{o}$, he will simultaneously observe a randomly sampled model play
...
\begin{align*}
  ...
  \underbrace{P_{o}(\messg \probbar \mystate{o})}_{\text{observe model play $\messg$}} \cdot
  \underbrace{\EU(\messg, \mystate{o}, \Rstrat)}_{\text{EU model choice in learner's view}}
\end{align*}
...
point $\messg$, a learner will observe a random model choose interpretation $\mystate{o}$
...
\begin{align*}
  ...
  \underbrace{P_{o}(\state \probbar \messg)}_{\text{observe model play $\state$}} \cdot
  \underbrace{\EU(\state, \messg, \Rstrat)}_{\text{EU model choice}}
\end{align*}
...
but eventually arbitrary) degree of stability. In other words, it is
a measure of relative speed capturing whether the system reached a state in which evolutionary
...
dynamic in terms of imitation of success. The resulting population-level dynamic produced
