...
would take us, and then apply diffusion. Since the latter step is the
...
\begin{align}
 ...
 \expo \left ( -  \frac{\card{\mystate{i} - \mystate{j}}^s}{ \impairment^2} \right ) \,,& \textrm{otherwise} \\
    \end{cases}
\end{align}
where $\impairment >= 0$ is an imprecision parameter. When $\impairment=0$ agents
perfectly discriminate between states; when $\impairment \rightarrow
...
\begin{figure}
  ...
  \caption{Examples for Nosofsky-similarity (non-normalized) and
    ...
    values of $\impairment$ and $\card{\States} = 50$.}
...
The example in Figure~\ref{fig:confusion-SenRec} shows that iterated
diffusion leads to sender and receiver behavior that is the same at
all choice points. After $n$ steps of diffusion an initial sender
strategy $\Sstrat$ will be $C^n \Sstrat$, and an initial receiver
strategy will be $\Rstrat C^n$. If we assume that $\impairment > 0$ and so
$C$ is a positive row-stochastic matrix (each state could in principle
be confused as any other state, albeit with possibly a very low
probability), it follows from the Perron-Frobenius theorem that the
limit $C^\infty = \lim_{n \rightarrow \infty} C^n$ exists (it is the
Perron-projection) and all of its rows are identical. That is why, in
the limit of iterated diffusion, $C^\infty S$ has identical rows
(messages are sent with the same probability in each state), and $R
C^\infty$ likewise has identical rows (every message is interpreted
...
\begin{itemize}
\item utility function: another Nosofsky-style function
\item get data for different set sizes, impairment and utility parameters
\item 
\end{itemize}
