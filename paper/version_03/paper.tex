...
\begin{figure}
  \centering
  \missingfigure{Please include here something like Figure 4 in your paper with Elliot.}
...
evolution.
We worked out the relation between the \rdd and the \rmd, but given that it is rather technical and not strictly necessary to appreciate the main point of the paper, we defer it to Appendix~\ref{sec:diffusion-as-special}.
In the following section we explore the long-term effects of using the \rdd.
...
= 1$ and $C_{ij} = 0$ for $i \neq j$. For $\impairment > 0$, $C$ is positive, i.e., $C_{ij} >0$ for
...
for reaching the maximum of 200 rounds. In contrast, the outcome of a trial
...
Figure~\ref{fig:MeanMetrics} shows plots summarizing some results.
%of our simulation.
For perspicuity, we only plot results for one level of
tolerance $\beta = 0.1$, and leave out the case of $n_s = 90$. Still,
every qualitative trend mentioned in the following applies to the
...
sender strategies obtained for the same parameter values, is plotted in Figure~\ref{fig:GroupMeasuresA} for $\beta =
