...
\title{Vagueness and imprecise imitation in signalling games}
...
\begin{abstract}
  ...
  categorisation. The upshot of this is that we see \emph{as-if}-generalisation at an aggregate
...
efficiency. \citet{Lipman2009:Why-is-Language} argues that, in common interest signalling
...
processing \citep[e.g.][]{FrankeJager2010:Vagueness-Signa} or of generalisation in low-level
...
Signalling games, as introduced by \citet{Lewis_1969:Convention}, have
...
project~\citep{CookKay2005:The-World-Color,WCS} collected colour naming data for 110 unwritten
languages of 45 language families.  For the great majority of these languages a pattern can be
observed: basic colour terms are by and large convex~\citep{Regier07,Jager2010}. It
...
\citet{JagerMetzger2011:Voronoi-Languag} is interesting because it demonstrates that signalling
...
There are at least two potential ways of interpreting the signalling setup.
...
module must discretise the information it is fed by selecting a small
...
other. In this way, \citeauthor{Gardenfors2000:Conceptual-Spac} argues, an efficient categorisation
...
senders and receivers in the population. A \emph{behavioural strategy} is a function that maps
an agent's choice points to a probability distribution over available
choices.\footnote{\label{fn:mixedStrats}Our focus is on behavioural strategies, not \emph{mixed
...
For example, in Robert Nosofsky's experiments~\citep{Nosofsky1986:Attention-Simil}, stimuli consisted of 16 semicircles with a radial line from the centre to the rim, varying in size (radius of 0.478, 0.500, 0.522, or 0.544 cm) and angle of the line (50º, 53º, 56º, or 59º).
In over 9000 trials, the two subjects identified the stimulus correctly only approximately 44\% and 35\% of the time.
Similar experiments have been conducted with other identification tasks, for example, for frequency and intensity of tones, taste, hue of colours, and magnitude of lines and areas~\citep{donkin_why_2015}.
...
expect a regularising effect also on evolving signalling strategies. Indeed, we hypothesise that
confusion of states can give rise to population-level aggregate behaviour that looks \emph{as
  if} information about what is good for one state percolates also to similar states. In other
words, imprecise imitation may make signalling behaviour on average look as if agents generalise
across similar states, even if no agent actually generalises. To explore whether confusion of
...
signal use. \emph{Convexity} captures whether a behavioural strategy would project onto a convex
...
the perceptual system) supports \emph{as-if}-generalisation at the population-level without
...
The main idea behind \emph{imitation of success} is that agents imitate the behaviour of other
