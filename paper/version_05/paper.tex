...
e.g., perception and language together, may be an almost optimal adaptation in a larger frame
...
natural cause of vagueness may have and that might counter some of the disadvantages that
...
behavior. In contrast, Figure~\ref{fig:efficientEq}Z shows a maximally efficient
...
would look like in Figure~\ref{fig:vagueUse}. This is not an equilibrium, but it gets close, so
...
\begin{figure}
  ...
  \begin{subfigure}[]{0.475\textwidth}
    ...
    \begin{tikzpicture}
      ...
      \draw[] (0,0) node (a1) {$\myact{1}$};

      \draw[] (1,0) node (a2) {$\myact{2}$};

      \draw[] (2,0) node (a3) {$\myact{3}$};

      \draw[] (3,0) node (a4) {$\myact{4}$};

      \draw[] (4,0) node (a5) {$\myact{5}$};

      \draw[] (5,0) node (a6) {$\myact{6}$};
...
\begin{subfigure}[]{0.475\textwidth}
  ...
  \caption{Examples of strategy pairs in sim-max games.}
