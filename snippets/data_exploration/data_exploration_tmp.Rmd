...
sp = ggplot(dsub, aes(x=Impairment, y=value, colour=Number.of.states)) + 
     geom_point(position = pd) +
     geom_errorbar(aes(ymin=value-ci, ymax=value+ci), width=0, position=pd) +
     geom_line(position=pd) + xlab("imprecision")

sp = sp + facet_grid(variable ~ Tolerance, scales="free")

show(sp)

pdf("~/Desktop/data/svn/vagueness-games/paper/plots/MeanMetrics.pdf", width=8, height = 6)
show(sp)
dev.off()
...
sp = sp + geom_bar(aes(fill = variable), color = "black", stat="identity", position= position_dodge(0.4), width = 0.35)
sp = sp + facet_grid(Number.of.states ~ Tolerance, scales="free") + ylab("proportion") + xlab("imprecision")

show(sp)

pdf("~/Desktop/data/svn/vagueness-games/paper/plots/CategoricalMeasures.pdf", width=8, height = 5)
show(sp)
dev.off()
