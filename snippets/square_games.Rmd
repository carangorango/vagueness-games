...
Inner category coherence
------------------------

Get inner category coherence to classify results

```{r}
icc = function(D,sen,theta=0.01){
  S = sen
  S[sen>theta] = 1
  S[sen<=theta] = 0
  S = prop.table(S,2)
  out = rep(0,dim(sen)[2])
  for (i in 1:dim(sen)[2]) {
    X = rowSums(D * S[,i])
    Y = sum(S[,i] * X)
    out[i] = Y
  }
...
P = matrix(0,ncol=2*ns*nm+2,nrow=trials)
...
for (t in 1:trials){
  ...
  icc.s = icc(D,sen)
  P[t,] = c(as.vector(c(sen, rec)),eu,icc.s)
}

Q = P
# P = P[,c(1:8,9,11,13,15,10,12,14,16,17,18,19)]
P = P[,c(17,18)]
