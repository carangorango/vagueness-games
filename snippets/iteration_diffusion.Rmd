...
plotSender = function(ns,nm,alphas,rounds){
  ...
  p = ggplot(senDF,aes(x = state, y = probability, color = message)) + 
      geom_point(size=0.5) + geom_line() + facet_grid(alpha~round) +
...
pdf(file = "paper/plots/confusion_sender.pdf", width = 7, height = 4)
show(pSen)
dev.off()

pdf(file = "paper/plots/confusion_receiver.pdf", width = 7, height = 4)
show(pRec)
dev.off()
