...
simDF = data.frame(distance = rep(states,2*length(alphas)),
                   ...
                   normalized = factor(rep(c("non-normalized", "normalized"),each=ns*length(alphas))),
                   similarity = c(as.vector(t(out)),as.vector(t(outNorm)))  )

p = ggplot(simDF, aes(x = distance, y = similarity, color=alpha_i)) + geom_point() +
  geom_line() + facet_grid(normalized~. , scales = "free") + ylab("") + xlab("distance in physical space")

show(p)

pdf('paper/plots/NosofskySim.pdf', width=7, height=4)
show(p)
dev.off()
```
